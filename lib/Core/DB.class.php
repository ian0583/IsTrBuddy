...
class Core_DB
{
	 ...
	 * DB constructor
	 *
	 * @param int $db - defined in DB.conf.php
	 * @return DB
	 */
	 ...
	 * @param boolean $returndata - if there is a result set to be returned
	...
	private function _execute($query, $returndata = true)
	{
		...
		$result = mysql_query($query) or die(mysql_error($this->_db) . "<br/>\nyour query: <b>". $query . '</b>');
	...
	/**
	 * Creates and sql query based on $query and $data array by replacing ? with corresponding $data values
	 *
	 * @param string $query
	 * @param array $data
	 * @return string - query
	...
	/**
	 * Automatically inserts/updates the $table with values from $data based on $index
	 *
	 * @param string $table
	 * @param array $data
	 * @param array $index
	...
	public function dataPrepare($db = DB_MAIN)
	{
		...
		foreach ($tables as $table)
		{
			...
			{
				$data['columns'][$column['Field']] = array('type' => $column['Type'], 'null_allowed' => $column['Null'], 'index_type' => $column['Key'], 'default_value' => $column['Default'], 'extra' => $column['extra']);
			}
			unset($columns);
