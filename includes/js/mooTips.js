var mooTips = new Class({
		...
		if (!this.options.tip)
		{
			this.options.tip = new Element('div', {
				...
				styles: {
					...
					left: '50%',
					top: '50%',
					'margin-left': ((this.options.width / 2) + this.options.padding) * (-1),
					'margin-top': ((this.options.height / 2) + this.options.padding) * (-1),
					*/
	...
	reposition: function(e)
	{
		...
		else
		{
			...
			{
				this.options.tip.setStyle('left', e.page.x - this.options.width - 10 - (this.options.padding * 2));
			}
			else
			{
				this.options.tip.setStyle('left', e.page.x + 10);
