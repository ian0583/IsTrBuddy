var mooCal = new Class({
	...
	createCalendar: function()
	{
		...
		this.calendarObject.tbody = new Element('tbody', {'class': this.options.tbodyClass}).injectInside(this.calendarObject.table);
		...
		for (var i = 1; i <= this.calData.daysofmonth; i++)
		{
			...
			var day = new Element('td', {'class': this.options.dayClass, 'title': this.months[d.get('mo')] + ' ' + i + ', ' + d.get('Fullyear')}).set('text', i).injectInside(tr);
	...
	{
		this.calendarObject.table = new Element('table', {'width': '100%', 'class': this.options.tableClass}).injectInside($(this.options.container));

		this.calendarObject.thead = new Element('thead', {'class': this.options.theadClass}).injectInside(this.calendarObject.table);

		this.calendarObject.monthRow = new Element('tr', {'class': this.options.monthClass}).injectInside(this.calendarObject.thead);

		var prev = new Element('td', {'align': 'center', 'class': this.options.prevClass}).set('text', '<<').injectInside(this.calendarObject.monthRow);
		new Element('td', {'colspan': 5, 'align': 'center', 'class': this.options.monthHeaderClass}).set('text', this.calData.month + ' ' + this.calData.year).injectInside(this.calendarObject.monthRow);
		var next = new Element('td', {'align': 'center', 'class': this.options.nextClass}).set('text', '>>').injectInside(this.calendarObject.monthRow);
		...
		this.calendarObject.headerRow = new Element('tr', {'class': this.options.dayofweekClass}).injectInside(this.calendarObject.thead);

		for (var i = 0; i < 7; i++)
		{
			new Element('td', {'align': 'center'}).set('text', this.dayofweek[i]).injectInside(this.calendarObject.headerRow);
		}
	},
...
var mooDay = new Class({
	...
	createDay: function()
	{
		...
		for (var i = 0; i < 24; i++)
		{
			...
			var tr = new Element('tr', {'class': this.options.rowClass}).injectInside(this.dayObject.tbody);

			var tdtime = new Element('td', {'align': 'center', 'width': '100px', 'class': this.options.timeClass}).set('text', time).injectInside(tr);
			var tdcontent = new Element('td', {'align': 'center', 'class': this.options.textClass}).set('text', '').injectInside(tr);
			...
			if (this.$events.click)
			{
				...
				{
					dayObj.$events.click[0](dayObj.options.date, this.get('title'), this.getChildren()[1].get('text'));
				});
			}
	...
	createHeader: function()
	{
		this.dayObject.table = new Element('table', {'width': '100%'}).injectInside($(this.options.container));
		...
		new Element('td', {'align': 'center', 'colspan': 2}).set('text', this.dayData.dayofweek + ' ' + this.options.date).injectInside(tr);
	},
	...
	setData: function(data)
	{
		...
		$$('.' + this.options.rowClass).each(function(el)
		{
